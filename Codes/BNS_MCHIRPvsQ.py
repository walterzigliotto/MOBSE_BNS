...
def plot(dataframe):
    ...
    for sim_numb in fMT:
        ...
        for Z in metallicities:
            ...
            im = sns.kdeplot(m_chirp(subsubset_dataframe), q(subsubset_dataframe), ax = ax[index//4,index%4] ,cmap = 'inferno', cbar = True, shade = True, shade_lowest = False, nlevels = 50)
            
            ax[index//4,index%4].set_ylabel("q", fontsize = 'x-large')
            ax[index//4,index%4].set_xlabel("$m_{chirp}$ $[M_\odot]$", fontsize = 'x-large')            
            ax[index//4,index%4].set_title("Z="+Z, fontsize = 'x-large')            
            index += 1
    
        fig.suptitle("q vs m_chirp density plots for fMT="+sim_numb, fontsize = 40)
