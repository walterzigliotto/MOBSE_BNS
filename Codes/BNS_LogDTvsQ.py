...
def plot(dataframe):
    ...
    for sim_numb, fmt_lab in zip(fMT, fMT_labels):
        ...
        fig, ax = plt.subplots(nrows = 3, ncols = 4, figsize = (24,18), sharex = True, sharey = True)
        index = 0
        ...
        for Z in metallicities:
            ...
            im = sns.kdeplot(q(subsubset_dataframe), np.log(subsubset_dataframe['tmerg[11]']), ax = ax[index//4,index%4] ,cmap = 'inferno', cbar = True, shade = True, shade_lowest = False, cbar_kws = {'label' : 'density'})
            
            ax[index//4,index%4].set_ylabel("log(delay time) [Myr]", fontsize = 'xx-large')
            ax[index//4,index%4].set_xlabel("q", fontsize = 'xx-large')            
            ax[index//4,index%4].set_title("Z="+Z, fontsize = 'xx-large')
            ax[index//4,index%4].tick_params(labelsize = 'x-large')
            
            im.figure.axes[-1].yaxis.label.set_size(15)
