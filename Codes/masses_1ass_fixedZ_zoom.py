...
def plot_assignment1_fixedZ(dataframe):
   ...
   #it returns the metallicities of the list above, but actually code is more flexible
    for fmt, fmt_lab in zip (list_values_fmT, fMT_labels):
        ...
        sns.distplot(subset_data['m1form[8]'], kde = False, label = simul_num, ax = axm1, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(subset_data['m1form[8]'], kde = False, label = simul_num, ax = axm1_zoom, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)

        #plot m2
        sns.distplot(subset_data['m2form[10]'], kde = False, label = simul_num, ax = axm2, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(subset_data['m2form[10]'], kde = False, label = simul_num, ax = axm2_zoom, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9},norm_hist=True)


        #plot mtot
        sns.distplot(m_tot, kde = False, label = simul_num, ax = axmtot, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(m_tot, kde = False, label = simul_num, ax = axmtot_zoom, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9},norm_hist=True)

        #plot q
        sns.distplot(q, kde = False, label = simul_num, ax = axq, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(q, kde = False, label = simul_num, ax = axq_zoom, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9})
        ...
        sns.distplot(m_chirp, kde = False, label = simul_num, ax = axmchirp_zoom, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9},norm_hist=True)



        #plot masses differences
        
        sns.distplot(diff1, kde = False, label = simul_num, ax = axdiff1, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(diff2, kde = False, label = simul_num, ax = axdiff2, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)

        sns.distplot(zams_1, kde = False, label = simul_num, ax = axzams_1, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
        sns.distplot(zams_2, kde = False, label = simul_num, ax = axzams_2, hist_kws={"histtype": "step", "linewidth" : 2, "alpha" : 0.9}, norm_hist=True)
    ...
    axm1.set_title("Distribution of m1 for Z = "+met_numb, fontsize = 20)

    axm2.set_xlabel("m2 $[M_\odot]$", fontsize = 20)
    axm2.set_ylabel("Density", fontsize = 20)
    axm2.set_title("Distribution of m2 for Z"+met_numb, fontsize = 20)

    axmtot.set_xlabel("$m_{tot}$ $[M_\odot]$", fontsize = 20)
    axmtot.set_ylabel("Density", fontsize = 20)
    axmtot.set_title("Distribution of $m_{tot} = m_1+m_2$ for Z = "+met_numb, fontsize = 20)

    axq.set_xlabel("q", fontsize = 20)
    axq.set_ylabel("Density", fontsize = 20)
    axq.set_title("Distribution of $q= m_2 / m_1$ for Z = "+met_numb, fontsize = 20)

    axmchirp.set_xlabel("$m_{chirp}$ $[M_\odot]$", fontsize = 20)
    axmchirp.set_ylabel("Density", fontsize = 20)
    axmchirp.set_title("Distribution of $m_{chirp}$ for Z = "+met_numb, fontsize = 20)

    axdiff1.set_xlabel("$\Delta$m1", fontsize = 20)
    axdiff1.set_ylabel("Density", fontsize = 20)
    axdiff1.set_title("Distribution of $\Delta$m1 for Z = "+met_numb, fontsize = 20)

    axdiff2.set_xlabel("$\Delta$m2", fontsize = 20)
    axdiff2.set_ylabel("Density", fontsize = 20)
    axdiff2.set_title("Distribution of $\Delta$m2 for Z = "+met_numb, fontsize = 20)

    axzams_1.set_xlabel("ZAMS 1", fontsize = 20)
    axzams_1.set_ylabel("Density", fontsize = 20)
    axzams_1.set_title("Distribution of ZAMS 1 for Z = "+met_numb, fontsize = 20)

    axzams_2.set_xlabel("ZAMS 2", fontsize = 20)
    axzams_2.set_ylabel("Density", fontsize = 20)
    axzams_2.set_title("Distribution of ZAMS 2 for Z = "+met_numb, fontsize = 20)

    axm1_zoom.set_xlabel("m1 $[M_\odot]$", fontsize = 20)
    axm1_zoom.set_ylabel("Density", fontsize = 20)
    axm1_zoom.set_title("Distribution of m1 for Z = "+metallicity, fontsize = 20) 
    axm1_zoom.set_xlim([1.15,1.5])

    axm2_zoom.set_xlabel("m2 $[M_\odot]$", fontsize = 20)
    axm2_zoom.set_ylabel("Density", fontsize = 20)
    axm2_zoom.set_title("Distribution of m2 for Z = "+metallicity, fontsize = 20) 
    axm2_zoom.set_xlim([1.15,1.5])
    
    axmtot_zoom.set_xlabel("$m_{tot}$ $[M_\odot]$", fontsize = 20)
    axmtot_zoom.set_ylabel("Density", fontsize = 20)
    axmtot_zoom.set_title("Distribution of $m_{tot} = m_1+m_2$ for Z = "+metallicity, fontsize = 20)
    axmtot_zoom.set_xlim([2.25,3.20])

    axmchirp_zoom.set_xlabel("$m_{chirp}$ $[M_\odot]$", fontsize = 20)
    axmchirp_zoom.set_ylabel("Density", fontsize = 20)
    axmchirp_zoom.set_title("Distribution of $m_{chirp}$ for Z = "+metallicity, fontsize = 20)
    axmchirp_zoom.set_xlim([1,1.35])

    axq_zoom.set_xlabel("q", fontsize = 20)
    axq_zoom.set_ylabel("Density", fontsize = 20)
    axq_zoom.set_title("Distribution of $q= m_2 / m_1$  for Z = "+metallicity, fontsize = 20)
    axq_zoom.set_xlim([0.3,1.4])
    ...
    axm1.legend(handles=temp_handles, title = 'fMT', labels=labels, fontsize = 16,title_fontsize = 16)
    
    
    handles, labels = axm2.get_legend_handles_labels()
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axm2.legend(handles=temp_handles,  title = 'fMT',labels=labels, fontsize = 16,title_fontsize = 16)
    
    handles, labels = axmtot.get_legend_handles_labels()
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axmtot.legend(handles=temp_handles, title = 'fMT', labels=labels, fontsize = 16,title_fontsize = 16)
    
    handles, labels = axq.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axq.legend(handles=temp_handles, title = 'fMT', labels=labels, fontsize = 16,title_fontsize = 16)

    handles, labels = axmchirp.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axmchirp.legend(loc = 'center right', title = 'fMT', handles=temp_handles, labels=labels, fontsize = 16,title_fontsize = 16)

    handles, labels = axdiff1.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axdiff1.legend(loc = 'center right', title = 'fMT', handles=temp_handles, labels=labels, fontsize = 16,title_fontsize = 16)

    handles, labels = axdiff2.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axdiff2.legend(loc = 'center right', handles=temp_handles, title = 'fMT', labels=labels, fontsize = 16,title_fontsize = 16)  

    handles, labels = axzams_1.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axzams_1.legend(loc = 'center right', title = 'fMT', handles=temp_handles, labels=labels, fontsize = 16,title_fontsize = 16)

    handles, labels = axzams_2.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor(), linewidth = 2) for h in handles]
    axzams_2.legend(loc = 'center right', title = 'fMT', handles=temp_handles, labels=labels, fontsize = 16,title_fontsize = 16) 
 
    handles, labels = axm1_zoom.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor()) for h in handles]
    axm1_zoom.legend(handles=temp_handles, labels=labels, title = 'fMT', fontsize = 16,title_fontsize = 16)

    handles, labels = axm2_zoom.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor()) for h in handles]
    axm2_zoom.legend(handles=temp_handles, labels=labels, title = 'fMT', fontsize = 16,title_fontsize=16)

    handles, labels = axmtot_zoom.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor()) for h in handles] 
    axmtot_zoom.legend(handles=temp_handles, labels=labels, title = 'fMT', fontsize = 16,title_fontsize = 16)

    handles, labels = axmchirp_zoom.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor()) for h in handles] 
    axmchirp_zoom.legend(handles=temp_handles, labels=labels, title = 'fMT', fontsize = 16,title_fontsize = 16)

    handles, labels = axq_zoom.get_legend_handles_labels()   
    temp_handles = [Line2D([], [], c=h.get_edgecolor()) for h in handles] 
    axq_zoom.legend(handles=temp_handles, labels=labels, title = 'fMT', fontsize = 16,title_fontsize = 16)
    ...
    output_dir = "masses_1assgn/fixed_Z/Z"+met_numb
    mkdir_p(output_dir)
